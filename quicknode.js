...
class SolanaFeeService {
  ...
  static async getHighPriorityFeePCU(lastNBlocks = 100) {
    try {
      ...
      const val = result?.per_compute_unit?.percentiles['95'];
      if (val) {
...
async function main() {
  ...
  if(!process.env.QUIKCNODE_RPC || !process.env.PRIVATE_KEY || !process.env.FEES_ENDPOINT || !process.env.SEND_TO_WALLET) {
    console.log('Please set QUICKNODE_RPC, SEND_TO_WALLET, FEES_ENDPOINT and PRIVATE_KEY in .env file');
    return
  }

  // Connect to Solana Mainnet
  const connection = new Connection(
    process.env.QUIKCNODE_RPC,
    'confirmed',
  );
